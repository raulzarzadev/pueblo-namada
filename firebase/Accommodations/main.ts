...
const AccommodationsCRUD = new FirebaseCRUD(
  'accommodations'
)
...
export const listenAllAccommodations = (
  cb: CallableFunction
...
export const getPlaceAccomodationsBettwenDates = (
  ...
  return AccommodationsCRUD.getMany([
    where(
      'dates.startsAt',
      '>=',
      Dates.toMiliseconds(startDate)
    ),
    where(
      'dates.startsAt',
      '<=',
      Dates.toMiliseconds(endDate)
    ),
    where('place', '==', placeId)
  ])
}
