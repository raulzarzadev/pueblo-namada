...
export const listenAllRoomRequests = (
  cb: CallableFunction
...
export const listenPlaceRoomRequests = (
  ...
  roomRequestsCRUD.listenDocs(
    [where('place', '==', placeId)],
    cb
  )

export const listenUserPlaceRoomRequests = (
  placeId: string,
  cb: CallableFunction
) => {
  const currentUserId = getAuth().currentUser?.uid
  return roomRequestsCRUD.listenDocs(
    [
      where('place', '==', placeId),
