...
const GuestPayments = ({ place, guest }) => {
  ...
  return (
    <div>
      ...
      {!payments?.length && (
        <p className='text-center'>Sin pagos aún</p>
      )}
      {payments?.map((payment) => {
        ...
        return (
          <div
            className='flex justify-evenly flex-col my-2 '
            key={payment.id}
          >
            ...
            <div className='sm:flex justify-between'>
              <div>
                Desde:
                {dates?.starts
                  ? formatDate(dates?.starts, 'dd/MM/yy')
                  : 'n/d'}
              </div>
              <div>
                Hasta:{' '}
                {dates?.ends
                  ? formatDate(dates?.ends, 'dd/MM/yy')
                  : 'n/d'}
              </div>
            </div>
